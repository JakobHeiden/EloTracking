...
public abstract class SlashCommand {
	...
	protected SlashCommand(ChatInputInteractionEvent event, EloTrackingService service, DiscordBotService bot, TimedTaskQueue queue) {
		this.event = event;
		this.service = service;
		this.bot = bot;
		this.queue = queue;
		this.guildId = event.getInteraction().getGuildId().get().asString();
